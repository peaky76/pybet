...
class TestOdds(TestCase):
    def test_odds_standard_fractionals_contains_odds_on(self):
        self.assertTrue("2/5" in FractionalOddsSets.STANDARD)

    def test_odds_standard_fractionals_contains_odds_against_short(self):
        self.assertTrue("5/2" in FractionalOddsSets.STANDARD)

    def test_odds_standard_fractionals_contains_odds_against_long(self):
        self.assertTrue("66/1" in FractionalOddsSets.STANDARD)
    ...
    def test_odds_add(self):
        self.assertAlmostEqual(
            Decimal(70.77),
            (Odds.percentage(40) + Odds.fractional(9, 4)).to_percentage(),
            places=2,
        )
    ...
    def test_odds_remain_odds_after_mul_with_odds(self):
        self.assertAlmostEqual(
            Decimal(12.31),
            (Odds.percentage(40) * Odds("3.25")).to_percentage(),
            places=2,
        )

    def test_odds_remain_odds_after_mul_with_decimal(self):
        self.assertAlmostEqual(
            Decimal(12.31), (Decimal(2.5) * Odds("3.25")).to_percentage(), places=2
        )
    ...
    def test_odds_to_fractional_with_standard_fractionals_constants(self):
        odds_set = FractionalOddsSets.STANDARD
        odds = Odds(5)
        self.assertEqual("4/1", odds.to_fractional(odds_set))

    def test_odds_to_fractional_uses_standard_fractionals_as_default_if_not_specified(
        self,
    ):
        odds = Odds(5)
    ...
    def test_odds_to_fractional_raises_error_with_incorrect_fractional_set(self):
        self.assertRaises(
            TypeError, lambda: Odds("4.30").to_fractional([(2), (4, 1), (6, 3, 1)])
        )
    ...
    def test_odds_to_percentage(self):
        self.assertAlmostEqual(
            Decimal("30.7692"), Odds.fractional(9, 4).to_percentage(), places=4
        )

    def test_odds_to_probability(self):
        self.assertAlmostEqual(
            Decimal("0.3077"), Odds.fractional(9, 4).to_probability(), places=4
        )
    ...
    def test_odds_lengthen(self):
        self.assertAlmostEqual(
            Decimal("3.3333"), Odds.percentage(40).lengthen(10), places=4
        )
